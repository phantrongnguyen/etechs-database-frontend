...
class PrivacySettings(BaseModel):
    ...
    @classmethod
    def validate_privacy_values(cls, v):
        if v is None:
            return "public"
        v_clean = v.strip().lower()
        allowed = ["public", "friends_only", "private"]
        if v_clean not in allowed:
            raise ValueError(f"Giá trị privacy phải thuộc một trong: {allowed}")
        return v_clean
...
class Onboarding(BaseModel):
    ...
    def clean_steps(cls, v):
        if not v:
            return []
        return [step.strip() for step in v if isinstance(step, str) and step.strip()]
